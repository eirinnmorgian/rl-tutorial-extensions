...
def cast_fireball(*args, **kwargs):
    ...
    results.append({
        'consumed': True,
        'message': Message('The fireball explodes, burning everything within {0} tiles'.format(radius), tcod.orange)
    })

    for entity in entities:
        if entity.distance(target_x, target_y) <= radius and entity.fighter:
            results.append({
                'message': Message('The {0} gets burned for {1} hit points'.format(entity.name, damage), tcod.orange)
            })
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            # compose confused ai into the entity
            confused_ai.owner = entity
            entity.ai = confused_ai

            results.append({
                'consumed': True,
                'message': Message('The eyes of the {0} go vacant as it starts to stumble about'.format(entity.name), tcod.light_green)
            })
